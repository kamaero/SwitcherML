...
```bash
./scripts/sync-version.sh       # Sync version from git code history
./scripts/install.sh            # Release build + install to /Applications
./scripts/install.sh --debug    # Debug build
./scripts/package.sh            # Build .pkg installer into ./dist
```

Versioning model:
- `CFBundleShortVersionString` and `CFBundleVersion` are auto-synced by `scripts/sync-version.sh`
- Version format: `MAJOR.MINOR.PATCH`
- `PATCH` = number of git commits that changed `Sources/SwitcherLM`
- Building the same commit does not increment version; new code commits do.
