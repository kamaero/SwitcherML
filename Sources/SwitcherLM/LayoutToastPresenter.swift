...
final class LayoutToastPresenter {
    ...
    private func showConversionToast(
        ...
        let width = badgeWidth + convWidth + 24
    ...
    private func cornerPoints(in frame: NSRect, size: NSSize, padding: CGFloat) -> [NSPoint] {
        [
            NSPoint(x: frame.minX + padding, y: frame.maxY - size.height - padding),
            NSPoint(x: frame.maxX - size.width - padding, y: frame.maxY - size.height - padding),
            NSPoint(x: frame.minX + padding, y: frame.minY + padding),
            NSPoint(x: frame.maxX - size.width - padding, y: frame.minY + padding),
        ]
    }
